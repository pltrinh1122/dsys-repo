...
### 15.9 Context transport — content-addressed manifest (DR-DIALOG-001)

The request's context is transported embed-by-value through a
content-addressed store (ratified 2026-09-19; reference-by-pin
killed — resolution in the ambient places the critical step
outside the governed boundary).

At issuance, the deterministic issuer resolves every context
pin to bytes and materializes them once per unique context:

```
var/dialog/context/<sha256>/
  manifest.json          # canonical entry list + hashes
  git/dsys-repo/<sha>/…       # pinned tree content
  git/accretion-repo/<sha>/…
  state/<state_ref>           # state snapshot
  transcript/<turn_id>/…      # transcript excerpts
  goal-store/<ref>
```

`prompt.json` embeds the manifest (not the bytes):

```json
"context_manifest": {
  "store": "<sha256 of canonical manifest>",
  "entries": [
    {"kind": "git", "repo": "dsys-repo", "commit": "<sha>",
     "path": "git/dsys-repo/<sha>", "sha256": "<sha>"},
    {"kind": "state", "ref": "<state_ref>",
     "path": "state/<state_ref>", "sha256": "<sha>"}
  ]
}
```
...
- **Generated by dsys, read by the ambient.** The ambient's read
  path is `var/dialog/context/<store>/`. The ambient never
  self-resolves pins.
- **Uniform:** always store, always manifest — one code path,
  no size-threshold special-casing.
- **Issuance validator (R6):** every scope entry resolvable at
  issuance; materialization hash recorded in the manifest;
  manifest embedded in `prompt.json`.
- **Ingest:** the response may carry `context_used`
  (author-declared list of store paths/hashes actually
  consulted — declared trust). Validators check shape only;
  manifest/`context_used` divergence is surfaced, not
  auto-failed.
- **Governance / audit-replay:** re-hash the store directory to
  verify byte-identity; an examiner reconstructs the exact
  context the inference was accountable to. (Audit-replay, not
  behavioral replay — §15, F3.)
- **GC:** un-designed (G6). Until designed, orphaned contexts
  are surfaced, not reaped — consistent with the dialog orphan
  policy (§9).
